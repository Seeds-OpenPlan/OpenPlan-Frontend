...
*/
/*
  bodyLines — 한 문장을 통째로 넘기고 브라우저가 알아서 접게 두면 "만드는 무/
  엇을" 처럼 어절 한가운데가 잘려서, 문장을 두 번 읽어야 하는 줄이 나왔다
  (오너 확인 2026-08-28). 그래서 줄바꿈 위치를 문구 자신이 들고 있게 바꾼다.
  끊는 자리는 의미 단위 — 쉼표 뒤, 그리고 1페이지는 "정해 두면" 뒤(조건절이
  끝나는 지점)로 오너가 지정했다. 1페이지 3줄, 2페이지 2줄.
  줄바꿈은 컴포넌트가 각 줄을 block으로 그려 만든다(<br> 아님) — 스크린리더가
  줄 단위로 끊어 읽지 않고 한 문단으로 읽게 하려는 것.
*/
export const INTRO_PAGES = [
  {
    ...
    bodyLines: [
      '프로젝트 안에 태스크를 만들고,',
      '예상 시간과 우선순위를 정해 두면',
      '언제 무엇을 해야 할지가 분명해집니다.',
    ],
  },
  {
    title: '주간 계획으로 시간에 배치해요',
    bodyLines: [
      '가용 시간과 고정 일정을 기준으로 태스크를 캘린더에 배치하고,',
      '수행 이력은 통계에서 다시 확인할 수 있습니다.',
    ],
  },
]
