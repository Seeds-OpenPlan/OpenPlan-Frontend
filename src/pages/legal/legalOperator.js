/*
  ...
  아래 값은 코드 어디에도 없다(운영 주체·책임자·연락처·시행일). 지어내면 그 순간
  법적 고지가 허위 사실이 되므로 한 파일에 모아 두고, 확정 전에는 화면에 확정 필요
  표시가 그대로 보이게 둔다 — 안 채운 채로 배포되면 눈에 띄어야 한다.

  구글 OAuth 심사는 이 페이지의 URL 을 직접 열어 확인한다. 확정 값을 채운 뒤
  배포해야 심사에 제출할 수 있다.

  ⚠️ cloudProvider·mailProvider 는 2026-09-01 에 저장소 근거로 확정했다(아래 주석).
     추정이 아니라 배포 산출물에서 읽은 값이므로 인프라가 바뀌면 함께 고쳐야 한다.
*/
const TBD = '[운영자 확정 필요]'
...
export const OPERATOR = {
  ...
  /*
    개인정보 보호책임자 성명·직책.
    🔴 2026-09-01: 운영 주체를 사용자 본인으로 확정했으나 방침에 실릴 실명이
       저장소 어디에도 없다(문서는 BE-1·FE-1 같은 역할 표기만 쓴다). 실명이
       정해지면 '홍길동 (팀 대표)' 형태로 채운다 — 성명 없이 직책만 적으면
       개인정보 보호법 제31조의 공개 요건을 만족하지 못한다.
  */
  privacyOfficer: TBD,
